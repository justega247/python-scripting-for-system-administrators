...
#
# The tool you’re building is going to be running on Linux systems, and it’s safe to assume that it’s going to run
# via sudo. With this information, it’s safe to say that the tool can utilize usermod, useradd, and userdel to keep
# users on the server up to date.
#
...
# I utilized the pwd module to get a list of all of the users on the system and determined which ones weren’t system
# users by looking for UIDs over 999 and ensuring that the user’s directory was under home. Additionally, the join
# method on str was used to combine a list of values into a single string separated by commas. This action is roughly
# equivalent to:
